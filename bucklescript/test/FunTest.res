...
let suite = suite("Fun", () => {
  ...
  test("flip", () => expect(Fun.flip(Int.\"/", 2, 4)) |> toEqual(Eq.int, 2))

  test("apply", () => expect(Fun.apply(a => a + 1, 1)) |> toEqual(Eq.int, 2))

  let increment = x => x + 1
  let double = x => x * 2
  test("compose", () => expect(Fun.compose(increment, double, 1)) |> toEqual(Eq.int, 3))
  ...
  test("composeRight", () => expect(Fun.composeRight(increment, double, 1)) |> toEqual(Eq.int, 4))
